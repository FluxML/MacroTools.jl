...
function flatten(x::Expr)
  if isexpr(x, :try) # for try blocks generated by the Julia parser, args can be either false or blocks, but not non-block exprs, except for the second argument that has to be false or symbol... so we need to be a bit verbose here
